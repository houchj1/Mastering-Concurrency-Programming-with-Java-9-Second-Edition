...
public class BlockingStatusTest {
    ...
    static void testBlockedByFileIO() throws InterruptedException {
         ...
         */
        Thread t1 = new Thread(() -> {
            try (RandomAccessFile file = new RandomAccessFile("D:\\test.txt", "rw");
                 FileChannel channel = file.getChannel();
                 FileLock lock = channel.lock()
                 ) {
                file.write(123);
                System.out.println("thread 1 write successfully");
                while (true) {
                    Thread.sleep(1000);
                }
        ...
        Thread t2 = new Thread(() -> {
            try (RandomAccessFile file = new RandomAccessFile("D:\\test.txt", "rw");
                ...
                file.write(123);
                System.out.println("thread 2 write successfully");
                System.out.println("thread 2 opens the file successfully!");
            } catch (FileNotFoundException e) {
                throw new RuntimeException(e);
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
        }, "thread-blocked-by-file-io");
        t2.start();
